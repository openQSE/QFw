...
import defw, logging, yaml, threading, select, traceback
import supermarq, random
import getopt
from defw_exception import DEFwCommError, DEFwInProgress, DEFwNotReady, DEFwError
...
def async_wait_read_cq(api, total_circ):
	...
	start = time()
	while (time() - start < circuit_run_timeout and \
...
def result_reader(total_circ, event_api):
	...
	logging.defw_app(f"Result reader thread ending. Events: {total_circuits_completed}. Expected: {total_circ}. Time: {time()}")
	for r in results:
...
def async_run_circuit(api, cb, start_qubits=20, num_shots=1, itr=30, increase=True, read_cq=True):
	...
	if not read_cq:
		...
		api.register_event_notification(me.my_endpoint(),
				EVENT_TYPE_CIRC_RESULT, event_api.class_id())
	...
	prformat(fg.orange+fg.bold,
			 f"****{itr} {start_qubits} qubit circuits completed in {time() - start_time}")
...
if __name__ == "__main__":
	...
	if len(sys.argv) >= 2:
		...
		try:
			options, args = getopt.getopt(argv, "b:m:y:s:q:o:i:u:c:t:h",
			["backend=", "method=", "run=", "increase=", "startqbit=", "shots=", "iterations=",
	...
	try:
		...
		if runtype == "sync":
			...
			async_run_circuit(qpm, op, start_qubits=startqbit,
				num_shots=num_shots, itr=iterations,
				increase=increase, read_cq=False)
		else:
