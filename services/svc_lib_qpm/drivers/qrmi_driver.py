...
class QrmiDriver(BaseDriver):
	...
	def run_circuit(self, circuit):
		...
		# Transcode it to an IQM circuit with the shared util, submit through
		# QRMI's task lifecycle, poll to completion, and normalize the counts to
		# qhw-result-v1 (the same normalizer the native svc_iqm_qpm path uses).
